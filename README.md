...
- 匿名发帖与评论：随机昵称 + 唯一身份密钥，凭密钥管理自己的内容，图文帖 + 匿名评论；作者可撤回自己的帖子/评论，撤回后退出列表与楼层、审核条目录下
...
| GET | /posts/{id} | 帖子详情 |
| DELETE | /posts/{id}/withdraw | 作者撤回帖子（需登录，仅作者本人） |
| GET | /posts/{id}/comments | 帖子评论 |
| POST | /comments | 发表评论（需登录） |
| DELETE | /comments/{id}/withdraw | 作者撤回评论（需登录，仅作者本人） |
| GET | /tags | 标签列表 |
| POST | /tags | 创建标签（需登录） |
